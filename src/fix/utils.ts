...
function findPatterns(text: string, pattern: string | string[], index: number = 0): number[][] {
 ...
 * @returns
 */
function dReplace(text: string, searchValue: string | RegExp, replaceValue: string) {
 ...
 * @returns
 */
function previousChar(text: string, index: number): string | null {
	// 从后往前进行正则匹配
...
let logger: object[] = []
export { findPatterns, previousChar, dReplace, dReplaceAt, dInsertAt, isNumber, logger }
